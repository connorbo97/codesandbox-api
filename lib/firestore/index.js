...
/*
	...
	Function:
		deletes directory from a sandbox
	Params:
		sandboxId: id of the sandbox document
...
const firestoreDeleteDirectory = async function(sandboxId, directoryId){
	...
	try{
		...
		let obj = {
			update_at: curDate.toISOString(),
			directories: omit([directoryId], data.directories),
		}
		firestore.collection('sandboxes').doc(sandboxId).update(obj)
		//TODO: delete all modules that are from this directory
		// firestore.collection('sandboxes').doc(sandboxId).collection('modules').doc(moduleId).delete()
		console.log(`Succesfully deleted directory ${directoryId} from ${sandboxId}`)
		return true
	} catch(err){
...
/*
	...
	Function:
		deletes module from a sandbox
	Params:
		sandboxId: id of the sandbox document
		moduleId: name of module to be deleted
	Return:
